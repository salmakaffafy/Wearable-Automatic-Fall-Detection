...
def handle_buzzer():
	...
	frequency = 2 if buzzerTime<5.0 else 4
	
	if(buzzerTime >= 10):
		buzzerFlag = False
		now = datetime.now()
		currentTime = now.strftime("%H:%M:%S")
		Client("utbdicrgcw9aqkmxweyq4ent9kxzbh").send_message("The Person has fallen and has not responded at " + currentTime, title="Emergency: Fallen Person")
		buzzerTime = 0
